...
# Use the correct function signature for modern OpenCV
ret, cameraMatrix, dist, rvecs, tvecs = cv.calibrateCamera(objpoints, imgpoints, frameSize, cameraMatrix=np.array([]), distCoeffs=np.array([]))
...
# mapx, mapy = cv.initUndistortRectifyMap(cameraMatrix, dist, np.eye(3), newCameraMatrix, (w,h), 5)
